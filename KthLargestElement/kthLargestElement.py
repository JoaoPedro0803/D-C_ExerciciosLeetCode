class Solution:
    ...
    def exotericSelect(self, nums: List[int], k: int) -> int:
        # Se o array tem apenas um elemento, retorne esse elemento
        if len(nums) == 1:
            return nums[0]
        
        # Obtendo um bom pivô a partir do algoritmo da mediana das medianas
        pivo = self.medianaDasMedianas(nums)
        
        # Dividindo os números em torno do pivô
        menoresQuePivo = [i for i in nums if i < pivo]
        maioresQuePivo = [i for i in nums if i > pivo]
        iguaisPivo = [i for i in nums if i == pivo]
        
        # Selecionando qual partição buscar
    ...
    def medianaDasMedianas(self, nums: List[int]) -> int:
        # Dividindo o array em sub-arrays de tamanho até 5
        subarrays = [nums[j:j+5] for j in range(0, len(nums), 5)]
        
        # Para cada sub-array, obtem a mediana
        medianas = [sorted(subarray)[len(subarray)//2] for subarray in subarrays]
        
        # Se o array de medianas tem 5 elementos ou menos, a mediana das medianas é a mediana do array de medianas
        if len(medianas) <= 5:
            return sorted(medianas)[len(medianas)//2]
        else:
